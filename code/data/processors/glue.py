...
class CustomProcessor(DataProcessor):
    """Processor for the Custom data set (GLUE version)."""
    ...
    def _read_csv(self, filepath):
        ...
        df = df.drop(df[(df['label']!=1) & (df['label']!=0) & (df['label']!=-1)].index)
    ...
    def get_test_examples(self, data_dir):
        """See base class."""
        print("Getting test examples from", data_dir)
        return self._create_examples(self._read_csv(os.path.join(data_dir)), "test")
    
    def get_examples_from_df(self, df):
        return self._create_examples(df.values.tolist(), 'df')
